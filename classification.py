...
model_tfidf_path = "/Users/sseung/Documents/study/python_class/real/txt/topicwise_top1000_tfidf.csv" #tfidf수치
predict_input_path = "/Users/sseung/Documents/study/python_class/real/txt/말뭉치_prompts_cleaned.csv" #입력
output_path = "/Users/sseung/Documents/study/python_class/real/txt/result.csv" #출력
...
def classify_article(text: str) -> str:
    ...
    pred = LogisticRegressionmodel.predict(vec)[0]
    return pred
...
probs = LogisticRegressionmodel.predict_proba(X_example)[0]
# for label, prob in zip(LogisticRegressionmodel.classes_, probs):
    # print(f"{label}: {prob*100:.2f}")

best_topic_index = probs.argmax()
best_topic = LogisticRegressionmodel.classes_[best_topic_index]
best_prob = probs[best_topic_index]

predicted_topic = best_topic
predicted_probability = best_prob

# print(f"\n✅ 가장 높은 분야: {best_topic} ({best_prob*100:.2f})")